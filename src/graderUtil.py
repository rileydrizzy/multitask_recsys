    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if self.is_hidden and not use_solution:
                ...
                args[0].skipTest(
                    "Hidden tests are skipped if the solution is not present."
                )
            return result
        ...
        def wrapper(*args, **kwargs):
            ...
            if os.name != "nt":
                ...
                def handle_timeout(signum, frame):
                    args[0].fail(
                        f"Test case timed out.  Max time: {self.maxSeconds} seconds"
                    )
...
class GradedTestCase(unittest.TestCase):
    ...
    def leaderboardColName(self):
        return getattr(
            getattr(self, self._testMethodName), "__leaderboard_col_name__", None
        )

    @property
    def leaderboardValue(self):
        return (
            self.__leaderboard_value__
            if hasattr(self, "__leaderboard_value__")
            else None
        )
    ...
    def studentFeedback(self):
        return getattr(
            getattr(self, self._testMethodName), "__student_feedback__", None
        )
    ...
    def elapsed(self):
        return (
            self.__elapsed__
            if hasattr(self, "__elapsed__")
            else time.perf_counter() - self.starttime
        )
...
class GradescopeTestResult(unittest.TestResult):
    ...
    def startTestRun(self):
        super().startTestRun()
        self.results = {"tests": [], "leaderboard": []}
    ...
    def storeResult(self, test, isSuccess, err=None):
        ...
        if test.leaderboardValue is not None:
            self.results["leaderboard"].append(
                {"name": test.leaderboardColName, "value": test.leaderboardValue}
            )
...
class StudentTestResult(unittest.TestResult):
    ...
    def stopTestRun(self):
        if not use_solution:
            self.stream.write(
                "Note that the hidden test cases are not shown here to check for correctness.\n"
            )
        self.stream.write(
            f"========== END GRADING [{self.earned_points}/{self.max_points} points + {self.earned_extra_credit}/{self.max_extra_credit} extra credit]\n"
        )
    ...
    def writeTestResults(self, test, isSuccess):
        earned = (
            "???"
            if test.isHidden and not use_solution
            else (test.earned if isSuccess else 0)
        )
        self.earned_points += test.earned if isSuccess and not test.isExtraCredit else 0
        self.earned_extra_credit += (
            test.earned if isSuccess and test.isExtraCredit else 0
        )
        hidden_blurb = (
            " (hidden test ungraded)" if test.isHidden and not use_solution else ""
        )
        self.stream.write(
            f"----- END {test.id()} [took {datetime.timedelta(seconds=test.elapsed)} (max allowed {test.timeout} seconds), {earned}/{test.weight} points]"
            + hidden_blurb
            + "\n\n"
        )
