...
class Test_1a(GradedTestCase):
    def setUp(self):
        ...
        self.sol_multitask_net = self.run_with_solution_if_possible(
            models, lambda sub_or_sol: sub_or_sol.MultiTaskNet
        )

    @graded()
    def test_0(self):
        """1a-0-basic:  single user and item embedding shape"""
        U, Q = self.multitask_net.init_shared_user_and_item_embeddings(
            self, self.num_users, self.num_items, self.embedding_dim
        )
        if U == None or Q == None:
            self.assertTrue(
                False, "init_shared_user_and_item_embeddings not implemented"
            )
        self.assertTrue(
            U.weight.shape == torch.Size([self.num_users, self.embedding_dim]),
            "incorrect tensor shape for self.U",
        )
        self.assertTrue(
            Q.weight.shape == torch.Size([self.num_items, self.embedding_dim]),
            "incorrect tensor shape for self.Q",
        )
    ...
    def test_1(self):
        ...
        (
            ...
            U_fact,
            Q_fact,
        ) = self.multitask_net.init_separate_user_and_item_embeddings(
            self, self.num_users, self.num_items, self.embedding_dim
        )
        if U_reg == None or Q_reg == None or U_fact == None or Q_fact == None:
            self.assertTrue(
                False, "init_separate_user_and_item_embeddings not implemented"
            )
        self.assertTrue(
            U_reg.weight.shape == torch.Size([self.num_users, self.embedding_dim]),
            "incorrect tensor shape for self.U_reg",
        )
        self.assertTrue(
            Q_reg.weight.shape == torch.Size([self.num_items, self.embedding_dim]),
            "incorrect tensor shape for self.Q_reg",
        )
        self.assertTrue(
            U_fact.weight.shape == torch.Size([self.num_users, self.embedding_dim]),
            "incorrect tensor shape for self.U_fact",
        )
        self.assertTrue(
            Q_fact.weight.shape == torch.Size([self.num_items, self.embedding_dim]),
            "incorrect tensor shape for self.Q_fact",
        )
    ...
    def test_2(self):
        ...
        self.assertTrue(
            B.weight.shape == torch.Size([self.num_items, 1]),
            "incorrect tensor shape for self.B",
        )
...
class Test_1b(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertTrue(
            type(mlp_layers[0]).__name__ == "Linear",
            "incorrect type for first layer of self.mlp_layers",
        )
        self.assertTrue(
            type(mlp_layers[1]).__name__ == "ReLU",
            "incorrect type for second layer of self.mlp_layers",
        )
        self.assertTrue(
            type(mlp_layers[2]).__name__ == "Linear",
            "incorrect type for third layer of self.mlp_layers",
        )
...
class Test_2(GradedTestCase):
    def setUp(self):
        ...
        self.multitask_net_with_sharing = models.MultiTaskNet(
            944, 1683, 32, [96, 64], True
        )
        self.multitask_net_without_sharing = models.MultiTaskNet(
            944, 1683, 32, [96, 64], False
        )
    ...
    def test_0(self):
        ...
        (
            predictions,
            score,
        ) = self.multitask_net_with_sharing.forward_with_embedding_sharing(
        ...
        self.assertTrue(
            ...
            "incorrect tensor shape for predictions",
        )
        self.assertTrue(
            score.shape
            == torch.Size(
            ...
            "incorrect tensor shape for score",
        )
    ...
    def test_1(self):
        ...
        (
            predictions,
            score,
        ) = self.multitask_net_without_sharing.forward_without_embedding_sharing(
            self.num_users, self.num_items
        )
        if predictions == None or score == None:
    ...
    def test_2(self):
        ...
        for epoch in range(self.total_epochs):
            factorization_loss, score_loss, joint_loss = self.model_with_sharing.fit(
                self.train
            )
        ...
        self.assertAlmostEqual(
            mrr, 0.06, delta=0.02, msg="mrr not converging to expected values"
        )
        self.assertAlmostEqual(
            mse, 0.9, delta=0.05, msg="mse not converging to expected values"
        )
    ...
    def test_3(self):
        ...
        mrr = mse = None
        for epoch in range(self.total_epochs):
            factorization_loss, score_loss, joint_loss = self.model_without_sharing.fit(
                self.train
            )
        ...
        self.assertAlmostEqual(
            mrr, 0.08, delta=0.02, msg="mrr not converging to expected values"
        )
        self.assertAlmostEqual(
            mse, 0.9, delta=0.05, msg="mse not converging to expected values"
        )
